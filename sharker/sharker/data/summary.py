...
class Stats:
    ...
    def from_data(
        cls,
        data: Union[List[int], List[float], ms.Tensor],
    ) -> Self:
...
class Summary:
    ...
    num_edges_per_type: Optional[Dict[Tuple[str, str, str], Stats]] = None
    ...
    def from_dataset(
        ...
        r"""Creates a summary of a :class:`~sharker.data.Graphset`
        object.
        ...
        for data in dataset:
            ...
            if per_type and isinstance(data, HeteroGraph):
                for node_type in data.node_types:
                    _num_nodes_per_type[node_type].append(data[node_type].num_nodes)
                for edge_type in data.edge_types:
                    _num_edges_per_type[edge_type].append(data[edge_type].num_edges)
