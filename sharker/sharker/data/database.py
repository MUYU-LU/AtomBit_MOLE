...
SORT_ORDER_TO_INDEX: Dict[str, int] = {
    None: -1,
    "row": 0,
    "col": 1,
}
...
class Database(ABC):
    ...
    .. code-block:: python
        ...
        db = Database()
        db[0] = Data(x=ops.randn(5, 16), y=0, z='id_0')
        print(db[0])
    ...
    .. code-block:: python
        ...
        db = Database(schema={  # Custom schema:
            ...
            'x': dict(dtype=ms.float32, size=(-1, 16)),
            'y': int,
            'z': str,
        })
        db[0] = dict(x=ops.randn(5, 16), y=0, z='id_0')
        print(db[0])
        >>> {'x': Tensor(...), 'y': 0, 'z': 'id_0'}
    ...
    def __init__(self, schema: Schema = object) -> None:
        ...
        self.schema: Dict[Union[str, int], Any] = {
            key: maybe_cast_to_tensor_info(value) for key, value in schema_dict.items()
        }
    ...
    def multi_insert(
        ...
        Args:
            indices (List[int] or Tensor or range): The indices at which
                to insert.
...
class SQLiteDatabase(Database):
    ...
    def multi_get(
        ...
        query = (
            f"CREATE TEMP TABLE {join_table_name} (\n"
            f"  id INTEGER,\n"
            f"  row_id INTEGER\n"
            f")"
        )
    ...
    def _serialize(self, row: Any) -> List[Any]:
        ...
        # * Tensor: Convert into the raw byte string
        # * object: Dump via pickle
        # If we find a `Tensor` that is not registered as such in
        ...
        for key, schema in self.schema.items():
            ...
            if isinstance(schema, TensorInfo):
                ...
                col = col.asnumpy()
                schema.dtype = col.dtype
                out.append(col.tobytes())
    ...
    def _deserialize(self, row: Tuple[Any]) -> Any:
        ...
        # * {int, float, str}: Use as they are.
        # * Tensor: Load raw byte string with `dtype` and `size`
        ...
        for i, (key, schema) in enumerate(self.schema.items()):
            ...
            if isinstance(schema, TensorInfo):
                if len(value) > 0:
                    tensor = np.frombuffer(value, dtype=schema.dtype)
                else:
                    tensor = np.empty(0, dtype=schema.dtype)
