...
class Batch(metaclass=DynamicInheritance):
    ...
    def index_select(self, idx: IndexType) -> List[Graph]:
        r"""Creates a subset of :class:`~sharker.data.Graph` or
        :class:`~sharker.data.HeteroGraph` objects from specified
        indices :obj:`idx`.
        ...
        if isinstance(idx, slice):
            ...
            index = mint.nonzero(idx).flatten().tolist()
    ...
    def to_data_list(self) -> List[Graph]:
        r"""Reconstructs the list of :class:`~sharker.data.Graph` or
        :class:`~sharker.data.HeteroGraph` objects from the
