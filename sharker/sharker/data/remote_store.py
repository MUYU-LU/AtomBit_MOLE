...
# integer index.
IndexType = Union[ms.Tensor, np.ndarray, slice, int]
# A representation of a feature tensor
FeatureTensorType = Union[ms.Tensor, np.ndarray]
ConversionOutputType = Tuple[
    Dict[Tuple[str, str, str], Tensor], Dict[Tuple[str, str, str], Tensor], Dict[Tuple[str, str, str], Optional[Tensor]]
]
...
class EdgeAttr:
    ...
    def __posi_init__(self):
        self.layout = EdgeLayout(self.layout)
        if self.layout == EdgeLayout.CSR and self.is_sorted:
            raise ValueError(
                "Cannot create a 'CSR' edge attribute with " "option 'is_sorted=True'"
            )
        if self.layout == EdgeLayout.CSC:
            self.is_sorted = True
...
class AttrView:
    ...
    def __getattr__(self, key: Any) -> Union["AttrView", FeatureTensorType]:
        ...
        for field in out._attr.__dataclass_fields__:
            if getattr(out._attr, field) == None:
                attr_name = field
                break
...
class FeatureStore(ABC):
    ...
    def put_tensor(self, tensor: FeatureTensorType, *args, **kwargs) -> bool:
        ...
        Args:
            tensor (Tensor or np.ndarray): The feature tensor to be
                added.
    ...
    def _to_type(
        ...
        if isinstance(attr.index, ms.Tensor) and isinstance(tensor, np.ndarray):
            return Tensor.from_numpy(tensor)
        if isinstance(attr.index, np.ndarray) and isinstance(tensor, ms.Tensor):
