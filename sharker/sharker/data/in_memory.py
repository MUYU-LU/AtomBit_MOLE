...
class InMemoryDataset(Dataset):
    ...
    def __init__(
        ...
        self._data: Optional[Graph] = None
        self.slices: Optional[Dict[str, Tensor]] = None
        self._data_list: Optional[MutableSequence[Optional[Graph]]] = None
    ...
    @classmethod
    def save(cls, data_list: Sequence[Graph], path: str) -> None:
        r"""Saves a list of data objects to the file path :obj:`path`."""
        data, slices = cls.collate(data_list)
        slices = to_array(slices)
        fs.pickle_save((data.numpy().to_dict(), slices, data.__class__), path)

    def load(self, path: str, data_cls: Type[Graph] = Graph) -> None:
        r"""Loads the dataset from the file path :obj:`path`."""
        out = fs.pickle_load(path)
    ...
    @staticmethod
    def collate(
        data_list: Sequence[Graph],
        return_tensor: bool = False
    ) -> Tuple[Graph, Optional[Dict[str, Tensor]]]:
        r"""Collates a list of :class:`~sharker.data.Graph` or
        :class:`~sharker.data.HeteroGraph` objects to the internal
        storage format of :class:`~sharker.data.InMemoryDataset`.
        """
        if len(data_list) == 1:
    ...
    def copy(self, idx: Optional[IndexType] = None) -> "InMemoryDataset":
        ...
        dataset.data, dataset.slices = self.collate(data_list, self.return_tensor)
        return dataset
    ...
    def to_on_disk_dataset(
        ...
        """
        if root is None and (self.root is None or not os.path.exists(self.root)):
            raise ValueError(
        ...
        root = root or os.path.join(self.root, "on_disk")

        in_memory_dataset = self
        ref_data = in_memory_dataset.get(0)
        if not isinstance(ref_data, Graph):
            raise NotImplementedError(
        ...
        class OnDiskDataset(OnDiskDataset):
            def __init__(
                self,
                root: str,
                transform: Optional[Callable] = None,
            ...
            def serialize(self, data: Graph) -> Dict[str, Any]:
                return data.to_dict()
    ...
    def __getattr__(self, key: str) -> Any:
        ...
        if isinstance(data, Graph) and key in data:
