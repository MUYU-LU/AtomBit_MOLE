...
from .graph import Graph, size_repr
from .storage import (
    BaseStorage,
    EdgeStorage,
    GlobalStorage,
...
class TemporalGraph(Graph):
    ...
    graph.
    The :class:`~sharker.data.TemporalGraph` object can hold a list of
    ...
    In general, :class:`~sharker.data.TemporalGraph` tries to mimic
    ...
    .. code-block:: python
        ...
        from sharker.data import TemporalData
    ...
    Args:
        src (Tensor, optional): A list of source nodes for the events
            with shape :obj:`[num_events]`. (default: :obj:`None`)
        dst (Tensor, optional): A list of destination nodes for the
    ...
    def __init__(
        self,
        src: Optional[ms.Tensor] = None,
        dst: Optional[ms.Tensor] = None,
        t: Optional[ms.Tensor] = None,
        msg: Optional[ms.Tensor] = None,
        **kwargs,
    ):
        super().__init__()
    ...
    def from_dict(cls, mapping: Dict[str, Any]) -> Self:
        r"""Creates a :class:`~sharker.data.TemporalGraph` object from
        a Python dictionary.
        """
        return cls(**mapping)

    def index_select(self, idx: Any) -> Self:
        idx = prepare_idx(idx)
        data = copy.copy(self)
    ...
    def __getattr__(self, key: str) -> Any:
        if "_store" not in self.__dict__:
            raise RuntimeError(
                "The 'data' object was created by an older version of MindGeometric. "
    ...
    def num_events(self) -> int:
        ...
        .. note::
            In a :class:`~sharker.data.TemporalGraph`, each row denotes
            an event.
    ...
    def num_edges(self) -> int:
        r"""Alias for :meth:`~sharker.data.TemporalGraph.num_events`."""
        return self.num_events
    ...
    def edge_index(self) -> ms.Tensor:
        ...
        if self.src is not None and self.dst is not None:
            return mint.stack([self.src, self.dst], dim=0)
        raise ValueError(
    ...
    def __inc__(self, key: str, value: Any, *args, **kwargs) -> Any:
        if "batch" in key and isinstance(value, ms.Tensor):
    ...
    def train_val_test_split(self, val_ratio: float = 0.15, test_ratio: float = 0.15):
        ...
        val_time, test_time = np.quantile(
            self.t.asnumpy(), [1.0 - val_ratio - test_ratio, 1.0 - test_ratio]
        )
