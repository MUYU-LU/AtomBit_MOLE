...
from .graph import Graph, warn_or_raise
from .heterograph import HeteroGraph
from ..utils import select, hyper_subgraph
...
class HyperGraph(Graph):
    ...
    This object differs from a standard :obj:`~sharker.data.Graph`
    ...
    def is_edge_attr(self, key: str) -> bool:
        ...
        if not val and self.edge_index is not None:
            return key in self and self[key].shape[0] == self.num_edges
        return val

    def __inc__(self, key: str, value: Any, *args: Any, **kwargs: Any) -> Any:
        if key == "edge_index":
            if isinstance(value, Tensor):
                return ms.Tensor([[self.num_nodes], [self.num_edges]])
    ...
    def subgraph(self, subset: Tensor) -> Self:
        ...
        Examples:
            ...
            ...     [0, 0, 1, 1, 1, 2, 2, 2]
            >>> ])
            >>> data = HyperGraph(x = x, edge_index = edge_index)
            >>> subset = Tensor([1, 2, 4])
        ...
        edge_index, _, edge_mask = hyper_subgraph(
            subset,
            self.edge_index,
            relabel_nodes=True,
    ...
    def has_isolated_nodes(self) -> bool:
        if self.edge_index is None:
            return False
        return mint.unique(self.edge_index[0]).shape[0] < self.num_nodes
        ...
        if self.edge_index is not None:
            if self.edge_index.dim() != 2 or self.edge_index.shape[0] != 2:
                ...
                warn_or_raise(
                    ...
                    f"'{cls_name}' (found {self.edge_index.shape})",
                    raise_on_error,
                )
