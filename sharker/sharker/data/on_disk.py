...
from .database import Database, RocksDatabase, SQLiteDatabase, Schema
...
class OnDiskDataset(Dataset):
    ...
    def serialize(self, data: Graph) -> Any:
        r"""Serializes the :class:`~sharker.data.Graph` or
        :class:`~sharker.data.HeteroGraph` object into the expected DB
        schema.
        """
    ...
    def deserialize(self, data: Any) -> Graph:
        r"""Deserializes the DB entry into a
        :class:`~sharker.data.Graph` or
        :class:`~sharker.data.HeteroGraph` object.
        """
        if self.schema == object:
            return data
        raise NotImplementedError(
    ...
    def __getitems__(self, indices: List[int]) -> List[Graph]:
