...
class Inspector:
    ...
    def _get_modules(self, cls: Type) -> List[str]:
        ...
        for base_cls in cls.__bases__:
            if base_cls not in {object, nn.Cell, MessagePassing}:
    ...
    def get_signature(
        ...
        if signature is None:
            raise IndexError(
                f"Could not access signature for function "
        ...
        for func in funcs:
            ...
            for param in params:
                ...
                if expected is not None and param.type != expected.type:
                    raise ValueError(
                        f"Found inconsistent types for argument "
                ...
                if expected is not None and param.default != expected.default:
                    if (
                        ...
                        raise ValueError(
                            f"Found inconsistent defaults for "
    ...
    def get_source(self, cls: Optional[Type] = None) -> str:
        ...
        if cls in {object, nn.Cell, MessagePassing}:
            return ""
    ...
    def get_params_from_method_call(
        ...
        if hasattr(self._cls, f"{func_name}_type"):
            ...
            if not isinstance(type_dict, dict):
                raise ValueError(
                    f"'{func_name}_type' is expected to be a "
            ...
            if match is not None:
                for arg in split(match, sep=","):
                    ...
                    if len(name_and_type_repr) != 2:
                        raise ValueError(
                            f"Could not parse argument '{arg}' "
            ...
            if match is not None:
                for i, kwarg in enumerate(split(match, sep=",")):
                    ...
                    if len(name_and_content) != 2:
                        raise ValueError(
                            f"Could not parse keyword argument "
