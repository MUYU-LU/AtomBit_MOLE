...
from .convert import from_scipy_sparse_matrix, to_scipy_sparse_matrix
...
def tree_decomposition(
    ...
    if len(edges) > 0:
        ...
        edge_index = ms.Tensor(edge_index_T).T
        inv_weight = 100 - ms.Tensor(weight)
        ...
        edge_index = ops.Tensor((2, 0)).long()

    rows = [[i] * len(atom2cliques[i]) for i in range(mol.GetNumAtoms())]
    row = ms.Tensor(list(chain.from_iterable(rows)))
    col = ms.Tensor(list(chain.from_iterable(atom2cliques)))
    atom2clique = mint.stack([row, col], dim=0).long()

    if return_vocab:
        vocab = ms.Tensor(xs).long()
