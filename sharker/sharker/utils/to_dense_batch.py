...
from . import scatter
from ..experimental import disable_dynamic_shapes, is_experimental_mode_enabled
...
def to_dense_batch(
    ...
    if batch is None and max_num_nodes is None:
        mask = mint.ones([1, x.shape[0]]).bool()
    ...
    num_nodes = scatter(
        ops.ones(x.shape[0], dtype=batch.dtype), batch, dim=0, dim_size=batch_size, reduce="sum"
    )
    ...
    tmp = mint.arange(batch.shape[0]) - cum_nodes[batch]
    ...
    size = (batch_size * max_num_nodes, ) + x.shape[1:]
    out = ms.Tensor(fill_value)
    out = out.astype(x.dtype).tile(size)
    out[idx] = x
    out = out.reshape((batch_size, max_num_nodes) + x.shape[1:])

    mask = mint.zeros(batch_size * max_num_nodes)
    mask[idx] = 1
